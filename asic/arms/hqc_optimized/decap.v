...
wire [31:0] m_in;
wire done_encap;
localparam LOG_M_WORDS = `CLOG2((K+(32-K%32)%32)/32);
reg [LOG_M_WORDS:0] m_addr;
...
reg copy_uv;
reg dec_uv;
wire [LOGX-1:0] uv_addr_0_dec;
wire [LOGX-1:0] uv_addr_1_dec;
...
`ifdef SHARED_ENCAP
    //outputs
    // AGENT FIX: decap's shake outputs are only driven by ENCAP_FOR_RENCRYPT,
    // which is compiled out under SHARED_ENCAP -> Z propagates into shared
    // Keccak during decrypt phase. Decrypt never uses SHAKE; tie off.
    assign shake_din_valid  = 1'b0;
    assign shake_din        = 32'b0;
    assign shake_dout_ready = 1'b0;
    assign shake_force_done = 1'b0;
