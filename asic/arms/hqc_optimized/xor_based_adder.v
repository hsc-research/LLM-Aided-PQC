...
module xor_based_adder
    ...
    output reg [LOG_DEPTH-1:0] in_addr_div,
    output reg in_rd_en,
...
reg [3:0] state     = 0;
localparam DIV_CDW = (parameter_set == "hqc128") ? 3 : 5;
reg [2:0] in_addr_rem;
        ...
        add_out_valid_reg <= 1'b0;
         ...
         if (state == s_wait_start) begin
            ...
            if (in_addr == DEPTH-1) begin
                ...
                in_addr <= in_addr + 1;
                if (in_addr_rem == DIV_CDW-1) begin
                    in_addr_div <= in_addr_div + 1;
                    in_addr_rem <= 0;
                end
                else begin
                    in_addr_rem <= in_addr_rem + 1;
                end
                state <= s_load_loc;
            end  
         end
